...
function generateRules
  echo rdr pass on lo0 inet proto tcp from any to any port 80 -\> (vmip) port 8080
  echo rdr pass on en0 inet proto tcp from any to any port 80 -\> (vmip) port 8080
  echo rdr pass on lo0 inet proto tcp from any to any port 443 -\> (vmip) port 8081
  echo rdr pass on en0 inet proto tcp from any to any port 443 -\> (vmip) port 8081
end;
